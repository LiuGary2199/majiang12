...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeCow != info.ComeCow)
        {
            isSame = false;
            Debug.Log("域名");
        }
...
#endif
        if (Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<MobTownEre>()[0].FadeCash != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADAnalyze>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
       ...
       // adjustObj.GetComponent<com.adjust.sdk.Adjust>().startManually = true;
        adjustObj.GetComponent<com.adjust.sdk.Adjust>().eventBuffering = false; //SDK v5 中已删除该设置。
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<CensusVoltAnalyze>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<CensusVoltAnalyze>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<CensusVoltAnalyze>();
            }
        }
        GameObject.Find("MainManager").GetComponent<CensusVoltAnalyze>().MotionID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<MoodDyAnalyze>())
        {
            GameObject.Find("MainManager").AddComponent<MoodDyAnalyze>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<MoodDyAnalyze>().There = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<MoodDyAnalyze>().There = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].FadeCash = info.GameCode;
        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeCow = info.ComeCow;
        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeHumidCow = info.ComeCow + CFellow.HumidCow;
        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeFellowCow = info.ComeCow + CFellow.FellowCow;
        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeRichCow = info.ComeCow + CFellow.RichCow;
        Resources.FindObjectsOfTypeAll<MobTownEre>()[0].ComeCensusCow = info.ComeCow + CFellow.CensusCow;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.ComeCow + CFellow.FellowCow + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.ComeCow + CFellow.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.ComeCow + CFellow.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) => {
           ...
           if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("MobAutoAnalyze"))
               {
                    DestroyImmediate(GameObject.Find("MobAutoAnalyze"));
               }
           }
       },
       () => {
