...
public class CashOutPanel : ComeUIFlank
{
    ...
    private void Start()
    {
        ...
        CloseBtn.onClick.AddListener(() => { MediaUIDeer(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = MobTownEre.instance.FellowLoom.CashOut_MoneyName;
        InfoText.text = MobTownEre.instance.FellowLoom.CashOut_Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = MobTownEre.instance.ComeCow;
        ContactBtn.onClick.AddListener(() => { Application.OpenURL(MobTownEre.instance.ComeCow); });
        ...
        {
            string url = MobTownEre.instance.ComeCow + "/privacy_policy.html";
        ...
        PolicyToggle.isOn = SameLoomAnalyze.OwnDeem("CashOut_PolicyAgree");
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            OughtAnalyze.GetInstance().BeadOught("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            OughtAnalyze.GetInstance().BeadOught(AccountErrorText.text);
            return;
        }

        CashOutManager.GetInstance().Account = AccountInput.text;
        SameLoomAnalyze.FadChange("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            OughtAnalyze.GetInstance().BeadOught("Please agree to the policy");
            return;
        }
        SameLoomAnalyze.FadDeem("CashOut_PolicyAgree", true);
    ...
    public void UpdateMoney()
    {
        ...
        SameLoomAnalyze.FadChange(CFellow.Dy_StageArabFlood, CashOutManager.GetInstance().Data.Cash.ToString("F2"));
        //进度条动画
        float MaxMoney = float.Parse(MobTownEre.instance.FellowLoom.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            OughtAnalyze.GetInstance().BeadOught("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        BaskTrialGerman.GetInstance().ArabTrial("1301", LeftTimeText.text, CashOutManager.GetInstance().Money.ToString());
    }
}
