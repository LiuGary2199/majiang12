...
public class FoodDram
{
    public static long Seminar()
    {
        return OwnRationale(DateTime.Now);
    }

    public static long OwnRationale(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string FoodRichArmory(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime EcologyBeFoodRich(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string EcologyArmory(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string EcologyArmory2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
