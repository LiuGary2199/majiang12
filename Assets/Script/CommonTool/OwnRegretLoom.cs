...
{
    public static void Amino<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class OwnRegretLoom : CopyVibration<OwnRegretLoom>
{
    // Start is called before the first frame update
    public bool LeftAdhere;
    public bool UpGrid;

    static public OwnRegretLoom instance;
    ...
    /// <returns></returns>
    public Vector3 MixShadeStage(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject OwnSubsideFadeTypify(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float StageBeProwl(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Prowl180Be360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int OwnPatchWonderful(Vector2 p1, Vector2 p2)
    {
        float angle = Prowl180Be360(StageBeProwl(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 MixDilemmaDiveSize(Renderer Renderer)
    {
    ...
    public Vector2 MixDilemmaDiveSize(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 MixNitricLoom(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 MixWidowTurnpike()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = MixBottomMason();
        float screenH = MixBottomDomain();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int MixLintelNomad(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] OwnRecipeTreasure(int total, int count)
    {
    ...
    public Dictionary<string, object> ChangeBeMineralize(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string MineralizeBeChange(Dictionary<string, object> Info)
    {
    ...
    public int YewFoodGift(string dateStart, DateTime dateEnd)
    {
    ...
    public int SecFoodGift(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan FoodGift(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 OwnOakAtSago(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = OwnOakAtSago(canvas, text, strFragmentIndex);
            Vector3 lastPos = OwnOakAtSago(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = OwnOakAtSago(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 OwnOakAtSago(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float MonumentByCate(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Kick= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string ImitateDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string MixtureDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Kick;
