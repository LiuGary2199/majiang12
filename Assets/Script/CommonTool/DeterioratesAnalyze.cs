...
/// 负责处理跨平台的通知功能
/// </summary>
public class DeterioratesAnalyze : MonoBehaviour
{
    public static DeterioratesAnalyze Overtone{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string NssDismal= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(EnhanceDeterioratesLeadership());
    }

    IEnumerator EnhanceDeterioratesLeadership()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void IlluminateRefrigeration()
    {
        RapidDeteriorates();
        IntentDeterioratesNonself();
    }

    public void RapidDeteriorates()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void IntentDeterioratesNonself()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void SendLightningDeteriorates(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void CurrencyDeteriorates(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(NssDismal))
            {
                Application.OpenURL(NssDismal);
                Debug.Log($"通过URL scheme({NssDismal})唤起游戏");
            }
        }
    }
    #endif
